...
import {
    createUser, createEnrollmentWithAddress, generateCreditCardData,
    createTicket, createTicketType, createRemoteTicketType, createTicketTypeWithoutHotel,
    createTicketTypeWithHotel, createHotel, createRoom
} from "../factories";
...
describe("GET hotel rooms", () => {
    ...
    describe("When token is valid", () => {
        ...
        it("Should respond with 404 if hotel do not exist", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const enrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketTypeWithHotel();
        ...
        it("Should respond with 200 and list hotel rooms", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const enrollment = await createEnrollmentWithAddress(user);
            const ticketType = await createTicketTypeWithHotel();
